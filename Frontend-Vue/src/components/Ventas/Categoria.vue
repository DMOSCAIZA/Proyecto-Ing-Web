  ...
  <v-layout align-start>
    <v-flex>
              ...
              <v-card-text>
                <v-container grid-list-md>
                  <v-layout wrap>
                    ...
                    <!--v-flex>
                      <v-text-field v-model="idcategoria " label="ID"></v-text-field>
                    </!--v-flex-->
                    <v-flex>
        ...
        <v-dialog v-model="adModal" max-width="290px">
          <v-card>
            ...
            <v-card-text>
              ...
              el item
              <p class="font-weight-bold">"{{ adnombre_categoria }}"</p>
            </v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
      ...
      >
        ...
        <template v-slot:[`item.opciones`]="{ item }">
          <v-btn fab dark x-small color="secondary">
            <v-icon class="mx-2" fab dark x-small @click="editItem(item)">
              mdi-border-color
            </v-icon>
          </v-btn>
            ...
            <template v-if="item.condicion">
              <v-btn class="mx-2" fab dark x-small color="error">
                <v-icon fab dark x-small @click="activarDesactivarMostrar(2, item)">
                  mdi-block-helper
                </v-icon>
              </v-btn>
            </template>
          ...
          <template v-else>
            <v-btn fab dark x-small class="mx-2" color="success">
              <v-icon fab dark x-small @click="activarDesactivarMostrar(1, item)">
                mdi-check-bold
              </v-icon>
            </v-btn>
          </template>
...
<script>
  ...
  export default {
    data: () => ({
      ...
      loading: false,
      headers: [
        { text: 'N° Categoria', value: 'idcategoria' },
    ...
    computed: {
      formTitle () {
        return this.editedIndex === -1 ? 'Nueva categoría' : 'Actualizando categoría'
      }
    },
    watch: {
      dialog (val) {
    ...
    methods:{
      ...
      activarDesactivarMostrar(accion,item){
        this.adModal=1;
        this.adnombre_categoria=item.nombre;
      ...
      activar(){
        //usamos axios
        let me=this;
        axios.put('api/Categorias/Activar/'+this.adidcategoria,{}).then(function(response){
          me.adModal=0;
          me.adAccion=0;
          me.adnombre_categoria="";
          me.adidcategoria="";
          me.listar();
        }).catch(function(error){
            console.log(error);
      ...
      desactivar(){
        let me=this;
        axios.put('api/Categorias/Desactivar/'+this.adidcategoria,{}).then(function(response){
          me.adModal=0;
          me.adAccion=0;
          me.adnombre_cat="";
          me.adidcategoria="";
