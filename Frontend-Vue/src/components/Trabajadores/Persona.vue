...
<script>
    ...
    export default {
        ...
        computed: {
            formTitle () {
                 return this.editedIndex === -1 ? 'Nueva Persona' : 'Actualizando Persona'
            }
        },
        ...
        methods:{
            listar(){
                let me =this;
                axios.get('api/Personas/Listar').then(function(response){
           ...
           validar(){
               this.valida=0;
               this.validaMensaje=[];
               if(this.nombre.length < 3 || this.apellidos.length > 40){
